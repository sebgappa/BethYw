...
*/
Measure::Measure(std::string codename, const std::string &label) : codename(codename), friendlyLabel(label) {}
...
*/
void Measure::setLabel(std::string const& label) {
    this->friendlyLabel = label;
}
...
*/
double Measure::getValue(unsigned int const & key) {
...
std::map<unsigned int, double> Measure::getAllValues() {
...
void Measure::setValue(unsigned int key, double value) {
    auto it = this->readings.find(key);
    if(it != this->readings.end()) {
        it->second = value;
        return;
    }

    this->readings.insert(std::make_pair(key, value));
}
...
*/
unsigned int Measure::size() noexcept {
...
bool operator==(const Measure &lhs, const Measure &rhs) {
    return lhs.readings.size() == rhs.readings.size() &&
           std::equal(lhs.readings.begin(), lhs.readings.end(), rhs.readings.begin()) &&
           lhs.friendlyLabel == rhs.friendlyLabel &&
