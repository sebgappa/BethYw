...
void Areas::setArea(std::string &localAuthorityCode, Area &area) {
    auto existingArea = this->areas.find(localAuthorityCode);
    auto newArea = new Area(localAuthorityCode);
    auto newNames = area.getAllNames();
    auto newMeasures = area.getAllMeasures();

    if(existingArea != this->areas.end()) {
        auto existingNames = existingArea->second->getAllNames();
        auto existingMeasures = existingArea->second->getAllMeasures();


        for(auto const& newName: newNames) {
            newArea->setName(newName.first, newName.second);
        }

        for(auto & newMeasure: newMeasures) {
            newArea->setMeasure(newMeasure.first, *newMeasure.second);
        }

        for(auto const& existingName: existingNames) {
            newArea->setName(existingName.first, existingName.second);
        }

        for(auto & existingMeasure: existingMeasures) {
            newArea->setMeasure(existingMeasure.first, *existingMeasure.second);
        }

        this->areas.erase(existingArea);
        this->areas.insert(std::make_pair(localAuthorityCode, newArea));
    } else {
        for(auto const& newName: newNames) {
            newArea->setName(newName.first, newName.second);
        }
...
*/
Area& Areas::getArea(std::string const& localAuthorityCode) {
    if(this->areas.find(localAuthorityCode) == this->areas.end()) {
        throw std::out_of_range("No area found matching " + localAuthorityCode);
    }
    return *this->areas[localAuthorityCode];
}
...
void Areas::populateFromAuthorityCodeCSV(
    ...
    size_t pos;
    std::string segment;
    std::vector<std::string> segmentsList;

    std::string headers;
    std::getline(is, headers);

    if(areasFilter != nullptr && !areasFilter->empty()) {
        for(const auto& code: *areasFilter) {
            segmentsList = {};
            while(std::getline(is, line)) {
                if(line.find(code) != std::string::npos) {
                    ...
                    segmentsList.push_back(line);
                }
            }

            Area area(segmentsList[0]);
            area.setName("eng", segmentsList[1]);
            area.setName("cym", segmentsList[2]);
            this->setArea(segmentsList[0], area);
...
void Areas::populateFromWelshStatsJSON(std::istream &is, const BethYw::SourceColumnMapping &cols,
    ...
    for (auto& el : j["value"].items()) {
        ...
        if((areasFilter->find(localAuthorityCode) != areasFilter->end()) || areasFilter->empty()) {

            std::string authorityNameEng = data[cols.at(BethYw::SourceColumn::AUTH_NAME_ENG)];
            area = Area(localAuthorityCode);
            ...
            if(cols.find(BethYw::SourceColumn::SINGLE_MEASURE_CODE) != cols.end() ||
               cols.find(BethYw::SourceColumn::SINGLE_MEASURE_NAME) != cols.end()) {
                if(data[cols.at(BethYw::SourceColumn::SINGLE_MEASURE_CODE)] != nullptr) {
                    measureCode = data[cols.at(BethYw::SourceColumn::SINGLE_MEASURE_CODE)];
                }
                if(data[cols.at(BethYw::SourceColumn::SINGLE_MEASURE_NAME)] != nullptr) {
                    measureName = data[cols.at(BethYw::SourceColumn::SINGLE_MEASURE_NAME)];
                }
            } else {
                if(data[cols.at(BethYw::SourceColumn::MEASURE_CODE)] != nullptr) {
                    measureCode = data[cols.at(BethYw::SourceColumn::MEASURE_CODE)];
                }
                if(data[cols.at(BethYw::SourceColumn::MEASURE_NAME)] != nullptr) {
            ...
            if((measuresFilter->find(measureCode) != measuresFilter->end()) || measuresFilter->empty()) {
                ...
                try{
                    ...
                    std::string valueAsString = data[cols.at(BethYw::SourceColumn::VALUE)];
                    value = stod(valueAsString);
                }
