...
std::vector<BethYw::InputFileSource> BethYw::parseDatasetsArg(
  ...
  if(inputDatasets.empty() || (std::find(inputDatasets.begin(), inputDatasets.end(), "all") != inputDatasets.end())) {
...
std::unordered_set<std::string> BethYw::parseAreasArg(
  ...
  if(inputAreas.empty() || (std::find(inputAreas.begin(), inputAreas.end(), "all") != inputAreas.end())) {
      return areas;
  }

  for (auto & areaCode : inputAreas) {
      areas.insert(areaCode);
  }
...
std::unordered_set<std::string> BethYw::parseMeasuresArg(
    ...
    if(inputMeasures.empty() || (std::find(inputMeasures.begin(), inputMeasures.end(), "all") != inputMeasures.end())) {
        return measures;
    }

    for (auto & measureCode : inputMeasures) {
        measures.insert(measureCode);
    }
...
std::tuple<unsigned int, unsigned int> BethYw::parseYearsArg(
    ...
    std::stringstream ss(inputYears);
    std::string segment;
    ...
    try {
        if(segmentsList.size() > 1) {
            years = std::make_tuple(std::stoi(segmentsList[0]), std::stoi(segmentsList[1]));
        } else {
            years = std::make_tuple(std::stoi(segmentsList[0]), std::stoi(segmentsList[0]));
        }
...
void BethYw::loadAreas(Areas& areas, const std::string &dir, const std::unordered_set<std::string> *areasFilter) {
    auto *file =  new InputFile(dir + InputFiles::AREAS.FILE);
    areas.populate(file->open(), BethYw::SourceDataType::AuthorityCodeCSV, InputFiles::AREAS.COLS, areasFilter);
}
...
void BethYw::loadDatasets(Areas& areas,
    ...
    auto datasets = new std::vector<BethYw::InputFileSource>(datasetsToImport);

    try{
        for(const auto& data: *datasets) {
            auto fileSource = new std::string(data.FILE);
            auto file = new InputFile(dir + *fileSource);

            areas.populate(file->open(), data.PARSER, data.COLS, areasFilter, measuresFilter, yearsFilter);
        }
